...
(function () {
  ...
  /* --- Anfrage-Art aus URL vorbelegen (Link von der Leistungen-Seite) --- */
  const anfrageSelect = document.getElementById('anfrage');
  if (anfrageSelect) {
    const anfrageParam = new URLSearchParams(window.location.search).get('anfrage');
    const matchingOption = anfrageParam && Array.prototype.find.call(
      anfrageSelect.options, function (opt) { return opt.value === anfrageParam; }
    );
    if (matchingOption) anfrageSelect.value = anfrageParam;
  }
